...
func (pm *ProtocolManager) Broadcast(msg interface{}, msgType uint64) {
	...
	if ok {
		pm.logger.Info("Start broadcast consensus message", "Height", v.Height, "BlockPartHeader", v.BlockPartsHeader.Hash, "msgType", msgType)
	} else {
...
func (pm *ProtocolManager) BroadcastTxs(txs types.Transactions) {
	...
	pm.logger.Info("Start broadcast transactions", "number of txs", len(txs))

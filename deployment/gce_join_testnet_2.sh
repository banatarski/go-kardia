...
# Docker images
KARDIA_GO_IMAGE=gcr.io/strategic-ivy-130823/go-kardia:testnet
...
do
  ...
  run_cmd="docker run -d -p ${PORT}:${PORT} -p ${PORT}:${PORT}/udp -p ${RPC_PORT}:${RPC_PORT} --name node${node_index} ${KARDIA_GO_IMAGE} --dev --mainChainValIndexes ${MAIN_CHAIN_VAL_INDEXES} --addr :${PORT} --name node${node_index} --rpc --rpcport ${RPC_PORT} --clearDataDir --peer ${peers}"

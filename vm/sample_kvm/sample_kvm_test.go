...
func TestExecuteInterContract(t *testing.T) {
	...
	r1, gas1, err1 := Call(addressA, setData, &Config{State: state})
	fmt.Println(gas1)
	fmt.Println(r1)
	if err1 != nil {
		t.Fatal(err1)
	...
	r, gas, err := Call(addressB, testData, &Config{State: state})
	fmt.Println(err)
	fmt.Println(r)
	fmt.Println(gas)

...
//	}
//
//	// TODO: this may be changed to check whether proposal receive enough block part
//
...
func (cs *ConsensusState) addVote(vote *types.Vote, peerID discover.NodeID) (added bool, err error) {
	cs.logger.Info("addVote", "voteHeight", vote.Height, "voteType", vote.Type, "valIndex", vote.ValidatorIndex, "csHeight", cs.Height)
	...
	case types.VoteTypePrevote:
		...
		cs.logger.Info("Added to prevote",
			...
			"hasTwoThirds", prevotes.HasTwoThirdsMajority(),
			"csStep", cs.Step,
		...
		if blockID, ok := prevotes.TwoThirdsMajority(); ok {
			cs.logger.Info("prevote - has 2/3")
			// There was a polka!
			...
			if !blockID.Hash.IsZero() && cs.ValidRound.IsLessThan(vote.Round) && vote.Round.Equals(cs.Round) {
		...
		if cs.Round.IsLessThan(vote.Round) && prevotes.HasTwoThirdsAny() {
			// Round-skip if there is any 2/3+ of votes ahead of us
			cs.enterNewRound(height, vote.Round)
		} else if cs.Round.Equals(vote.Round) && cstypes.RoundStepPrevote <= cs.Step { // current round
	...
	case types.VoteTypePrecommit:
		...
		if ok {
			cs.logger.Info("ok", "Hash", blockID.Hash.Hex())
			cs.enterNewRound(height, vote.Round)
			cs.enterPrecommit(height, vote.Round)

			if !blockID.Hash.IsZero() {
				cs.logger.Info("blockId is not empty")
...
func (cs *ConsensusState) signVote(type_ byte, hash cmn.Hash, header types.PartSetHeader) (*types.Vote, error) {
	...
	// Simulate voting strategy
	// TODO(kiendn): remove this, because this is used for simulating purpose only
...
func (cs *ConsensusState) addProposalBlockPart(msg *BlockPartMessage, peerID discover.NodeID) (added bool, err error) {
	...
	if added && cs.ProposalBlockParts.IsComplete() {
		...
		for i:=uint(0);i<cs.ProposalBlockParts.Total();i++{
			encodedBlock = append(encodedBlock, cs.ProposalBlockParts.GetPart(int(i)).Bytes.Bytes()...)
		}

		if err := rlp.DecodeBytes(encodedBlock, &newBlock); err != nil {
			return added, err
		}
		cs.ProposalBlock = newBlock
		// NOTE: it's possible to receive complete proposal blocks for future rounds without having the proposal
		cs.logger.Info("Received complete proposal block", "height", cs.ProposalBlock.Height(), "hash", cs.ProposalBlock.Hash())
...
func (cs *ConsensusState) enterPrecommitWait(height *cmn.BigInt, round *cmn.BigInt) {
	...
	if !cs.Height.Equals(height) || round.IsLessThan(cs.Round) || (cs.Round.Equals(round) && cs.TriggeredTimeoutPrecommit) {
		logger.Debug(cmn.Fmt("enterPrecommitWait(%v/%v): Invalid args. "+
...
func (cs *ConsensusState) enterCommit(height *cmn.BigInt, commitRound *cmn.BigInt) {
	...
	if !cs.ProposalBlock.HashesTo(blockID) {
		if !cs.ProposalBlockParts.HasHeader(blockID.PartsHeader) {
			logger.Info("Commit is for a block we don't know about. Set ProposalBlock=nil", "commit", blockID.Hash)
			// We're getting the wrong block.
...
func (cs *ConsensusState) tryFinalizeCommit(height *cmn.BigInt) {
	...
	if !cs.ProposalBlock.HashesTo(blockID) {
		logger.Info("Attempt to finalize failed. We don't have the commit block.", "proposal-block", blockID.String(), "commit-block", blockID)
		return
	}
...
func (cs *ConsensusState) finalizeCommit(height *cmn.BigInt) {
	...
	var err error
	stateCopy, err = state.ApplyBlock(cs.logger, stateCopy, types.BlockID{Hash: block.Hash(), PartsHeader: blockParts.Header()}, block)
	if err != nil {
...
// error.
func (cs *ConsensusState) createProposalBlock() (block *types.Block, blockPart *types.PartSet) {
	...
	proposer := cs.privValidator.GetAddress()
	return cs.blockOperations.CreateProposalBlock(cs.Height.Int64(), cs.state, commit, proposer)
}
...
func (cs *ConsensusState) handleMsg(mi msgInfo) {
	...
	case *BlockPartMessage:
		...
		if added {
			//cs.statsMsgQueue <- mi
			cs.logger.Info(fmt.Sprintf("%v has been added to peerID %v", msg.String(), peerID))
		}

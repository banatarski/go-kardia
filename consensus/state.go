...
func (cs *ConsensusState) decideProposal(height *cmn.BigInt, round *cmn.BigInt, cblock *types.Block, cblockParts *types.PartSet) {
	...
	if err := cs.privValidator.SignProposal(cs.state.ChainID, proposal); err == nil {
		...
		cs.logger.Info("Signed proposal", "height", height, "round", round, "proposal", proposal.BlockPartsHeader.String())
...
func (cs *ConsensusState) addProposalBlockPart(msg *BlockPartMessage, peerID discover.NodeID) (added bool, err error) {
	...
	// In case we're not expecting a block part
	...
	if added && cs.ProposalBlockParts.IsComplete() {
		// Block parts added and completed
...
func (cs *ConsensusState) doPrevote(height *cmn.BigInt, round *cmn.BigInt) {
	...
	if cs.ProposalBlock == nil {
		...
		cs.signAddVote(types.VoteTypePrevote, common.NewZeroHash(), types.NilPartSetHeader())
		return
	}
...
func (cs *ConsensusState) enterPrecommit(height *cmn.BigInt, round *cmn.BigInt) {
	...
	if !ok {
		...
		cs.signAddVote(types.VoteTypePrecommit, common.NewZeroHash(), types.NilPartSetHeader())
		return
	}
...
func (cs *ConsensusState) tryFinalizeCommit(height *cmn.BigInt) {
	...
	if !cs.ProposalBlock.HashesTo(blockID.Hash) {
		logger.Info("Attempt to finalize failed. We don't have the commit block.", "proposal-block", cs.ProposalBlock.Hash())
		return
	}

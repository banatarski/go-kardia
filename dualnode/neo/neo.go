...
var (
	OneNeo                = big.NewInt(1).Exp(big.NewInt(10), big.NewInt(18), nil) // 10**18 is 1 neo in exchange smc
...
func (n *NeoProxy) SubmitTx(event *types.EventData) error {
	...
	case configs.CompleteFunction:
		// TODO(@sontranrad): these logic is outdated as we have new logic for exchange
		// and should  be remove after tested

...
import (
	...
	"github.com/kardiachain/go-kardia/lib/crypto/merkle"
...
type Part struct {
	...
	Bytes common.HexBytes    `json:"bytes"`
...
func NewPartSetFromData(data []byte, partSize int) *PartSet {
	...
	for i := 0; i < total; i++ {
		part := &Part{
			Index: uint(i),
			Bytes: data[i*partSize : common.MinInt(len(data), (i+1)*partSize)],
		}
		parts[i] = part
		partsBytes[i] = part.Bytes
		partsBitArray.SetIndex(part.Index, true)
	}

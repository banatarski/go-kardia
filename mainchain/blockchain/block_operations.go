...
func NewBlockOperations(logger log.Logger, blockchain *BlockChain, txPool *txpool.TxPool) *BlockOperations {
...
func (bo *BlockOperations) CreateProposalBlock(height uint64, lastBlockID common.Hash,
	lastValidatorHash common.Hash, commit *types.Commit) (block *types.Block, blockParts *types.PartSet, err error) {
	...
	bo.logger.Trace("Make block to propose", "block", block, "blockParts", blockParts)
...
func (bo *BlockOperations) SaveBlock(block *types.Block, seenCommit *types.Commit, blockParts *types.PartSet, proposal *types.Proposal) {
	if block == nil {
	...
	if !blockParts.IsComplete() {
		common.PanicSanity(common.Fmt("BlockOperations can only save completed block's part sets"))
	}
	...
	// TODO: NEED TO COMBINE ALL BLOCK PARTS HERE TO WRITE TO DB
...
func (bo *BlockOperations) newHeader(height uint64, numTxs uint64, blockID common.Hash, validatorsHash common.Hash) *types.Header {
	return &types.Header{
...
func (bo *BlockOperations) collectTransactions() []*types.Transaction {
	...
	//@luu: We arealy sort tx in txpool, so no need to double check here
	return pendingTxs
}

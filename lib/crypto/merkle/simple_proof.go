...
type SimpleProof struct {
	Total    uint     `json:"total"`     // Total number of items.
	Index    uint      `json:"index"`     // Index of item to prove.
...
// Recursive impl.
func computeHashFromAunts(index uint, total uint, leafHash []byte, innerHashes [][]byte) []byte {
...
func trailsFromByteSlices(items [][]byte) (trails []*SimpleProofNode, root *SimpleProofNode) {
	...
	default:
		k := getSplitPoint(uint(len(items)))
